...
- Added option-1 Windows signing path (Azure Artifact Signing via OIDC) while keeping `.pfx` fallback:
  - updated CI Windows packaging job in `.github/workflows/ci.yml`
  - updated release Windows job in `.github/workflows/release.yml`
  - Artifact Signing now runs post-package when `AZURE_*` secrets are present
  - release workflow rebuilds portable zip from signed binaries when Artifact Signing is enabled
  - `.pfx` signing remains as fallback if Azure signing secrets are not set
- Updated docs/backlog for new signing path:
  - `docs/DESKTOP_WRAPPER.md` (recommended Azure flow + fallback docs)
  - `docs/APP_METADATA_AUDIT.md` note updated
  - `TODO.md` unresolved Windows signing item now tracks Azure Artifact Signing provisioning
