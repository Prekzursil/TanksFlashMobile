...
function buildConfiguration(): Configuration {
  ...
  configuration.setBatch(
    new BatchInfo(
      process.env.APPLITOOLS_BATCH_NAME || `TanksFlashMobile-${process.env.GITHUB_SHA || 'local'}`,
    ),
  );
...
test('capture wrapper shell states with Applitools', async ({ page }) => {
  ...
  try {
    ...
    const payload = {
      unresolved: numberOrZero(
        (closeResult as any)?.getUnresolved?.() ?? (closeResult as any)?.unresolved,
      ),
      mismatches: numberOrZero(
        (closeResult as any)?.getMismatches?.() ?? (closeResult as any)?.mismatches,
      ),
